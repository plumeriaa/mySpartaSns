...
def user_follow(request, id):
    me = request.user   # me 나.
    click_user = UserModel.objects.get(id=id)  # click_user 내가 팔로우하고자 클릭한 사람.
    if me in click_user.followee.all():
        click_user.followee.remove(request.user)  # 이미 팔로우됨.
    else:
        click_user.followee.add(request.user)  # 팔로우 안되었으니 팔로우 요청.
